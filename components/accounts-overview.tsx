...
const initialAccounts = [
  { name: "Checking", balance: 750000 },
  { name: "Savings", balance: 5600000 },
...
export function AccountsOverview() {
  ...
  const handleRequestMoney = (amount, contact) => {
    console.log(`Requested KSh${amount} from ${contact.name}`)
  }
        ...
        <div className="mt-4 space-y-2">
          {accounts.map((account) => (
            <div key={account.name} className="flex justify-between items-center">
              ...
              <span className="text-sm font-medium">KSh{account.balance.toLocaleString()}</span>
            </div>
          ))}
        </div>
